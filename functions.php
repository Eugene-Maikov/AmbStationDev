<?php
/**
 * ambulance functions and definitions
 *
...
function ambulance_setup() {
	/*
		...
		* If you're building a theme based on ambulance, use a find and replace
		* to change 'ambulance' to the name of your theme in all the template files.
		*/
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'ambulance' ),
		)
	);
...
add_action( 'after_setup_theme', 'ambulance_setup' );
...
function ambulance_scripts() {
	wp_enqueue_style( 'ambulance-style', get_stylesheet_uri(), array(), _S_VERSION );
	wp_style_add_data( 'ambulance-style', 'rtl', 'replace' );

	wp_enqueue_script( 'ambulance-navigation', get_template_directory_uri() . '/js/navigation.js', array(), _S_VERSION, true );
